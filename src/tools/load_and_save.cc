/* An example C program that loads an mpd document and writes it back to
** disk.  For new files, this acts as a copy function. For old files, the
...
int main(int argc, char **argv)
{
    ...
    /* set the active file for fileread */
    set_file(f);

    /* make it a document file */
    put_filecode(DOCUMENTFILE);

    /* save the document */
    save_editwindow(windowdata);

    /* unset the active file */
    unset_file();
